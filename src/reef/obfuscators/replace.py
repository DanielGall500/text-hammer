...
class ReplaceObfuscator(Obfuscator):
    ...
    def obfuscate(self, doc: Doc, algorithm: Algorithm = "nouns-only", replace_with_pos:bool=False) -> str:
        match algorithm:
            case "nouns-only":
    ...
    def _keep_only(self, doc: Doc, pos_tags: list[POS], replace_with_pos:bool=False) -> str:
        remaining_tokens = []
        for token in doc:
    ...
    def _keep_all_except(self, doc: Doc, pos_tags: list[POS], replace_with_pos:bool=False) -> str:
        remaining_tokens = []
        for token in doc:
